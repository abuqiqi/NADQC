...
class PartitionAssigner(ABC):
    ...
    @abstractmethod
    def assign(self, partition_candidates: Any) -> dict[str, Any]:
        """
        处理量子线路划分计划
    ...
    def _extract_initial_partition_plan(self, 
                                        partition_candidates: list[list[list[list[int]]]]) -> list[list[list[int]]]:
        """
        ...
        for i in range(len(partition_candidates)):
            partition_plan.append(partition_candidates[i][0])  # 仅选择第一个划分方案
        return partition_plan
    
    def _evaluate_partition_plan(self, partition_plan: list[list[list[int]]]) -> dict[str, Any]:
        """
        评估划分计划的性能指标
...
class DirectPartitionAssigner(PartitionAssigner):
    """
    ...
    def assign(self, partition_candidates: Any) -> dict[str, Any]:
        ...
        # 提取初始划分计划
        partition_plan = self._extract_initial_partition_plan(partition_candidates)
        ...
        return {
            ...
            "costs": {
                ...
                "processing_time (sec)": end_time - start_time,
            }
        }


class MaxMatchPartitionAssigner(PartitionAssigner):
    """
    ...
    def assign(self, partition_candidates: Any) -> dict[str, Any]:
        ...
        for t in range(1, len(partition_plan)):
            ...
            new_curr_assign = [[] for _ in range(num_qpus)]
            for i in range(num_qpus):
                # 将当前时间步的逻辑QPU j 映射到新的逻辑QPU位置 curr_idx[i]
                # print(f"Time step {t}: matched prev QPU {prev_idx[i]} with curr QPU {curr_idx[i]}, intersection size = {weights[prev_idx[i]][curr_idx[i]]}")
...
class GlobalMaxMatchPartitionAssigner(PartitionAssigner):
    """
    ...
    def assign(self, partition_candidates: Any) -> dict[str, Any]:
        ...
        if k < 2:
            return {
                "partition_plan": partition_plan,
                "costs": {
                    "processing_time (sec)": time.time() - start_time,
        ...
        # 回溯路径
        orders = []
        orders.append(all_perms[best_last_idx])
        
        curr_idx = best_last_idx
        for t in range(k-1, 0, -1):
            prev_idx = path[t][curr_idx]
            orders.append(all_perms[prev_idx])
            curr_idx = prev_idx
